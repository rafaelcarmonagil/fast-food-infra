...
resource "aws_sqs_queue" "notificacao-pagamento-error" {
  name = var.sqs_notificacao_pagamento_error
  sqs_managed_sse_enabled = false
}

resource "aws_sqs_queue_policy" "sqs_notificacao_pagamento_error-policy" {
  queue_url = aws_sqs_queue.notificacao-pagamento-error.id
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "sqs:*",
      "Resource": "${aws_sqs_queue.notificacao-pagamento-error.arn}"
    }
  ]
}
POLICY
